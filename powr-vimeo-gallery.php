<?php
    ...
    Plugin URI: https://www.powr.io/plugins/vimeo-gallery
    Description: Get more views on Vimeo with embeddable playlists  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Vimeo_Gallery extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Vimeo Gallery.
        </p>
        <p>
    ...
    function powr_vimeo_gallery_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-vimeo-gallery' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-vimeo-gallery' label='$label'></div>";
      }else{
      	"<div class='powr-vimeo-gallery'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_vimeo_gallery_menu' );
    function powr_vimeo_gallery_menu() {
      add_menu_page( 'POWr Vimeo Gallery', 'POWr Vimeo Gallery', 'manage_options', 'powr-vimeo-gallery-settings', 'powr_vimeo_gallery_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_vimeo_gallery_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/vimeo-gallery';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/vimeo-gallery';
      }
    ...
    register_activation_hook(__FILE__, 'powr_vimeo_gallery_plugin_activate');
      add_action('admin_init', 'powr_vimeo_gallery_plugin_redirect');

      function powr_vimeo_gallery_plugin_activate() {
      add_option('powr_vimeo_gallery_plugin_do_activation_redirect', true);
      }
      ...
      function powr_vimeo_gallery_plugin_redirect() {
      if (get_option('powr_vimeo_gallery_plugin_do_activation_redirect', false)) {
          delete_option('powr_vimeo_gallery_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-vimeo-gallery-settings&' );
          }
       }
      }
